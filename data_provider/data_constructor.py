...
def construct_dataset(code, index_code_list, predict_days=5, threshold=0.05, append_index=True, append_history=True,
    ...
    _add_predicts_days(csv_data, predict_days, threshold=threshold)
...
def _add_predicts_days(csv_data, predict_days, threshold=0.0):
...
def _save_temp_data_to_csv_file(csv_data, title_list):
    positive_data = csv_data[csv_data.predict_sort == 1.0]
    save_temp_positive_data(positive_data, title_list)
    negative_data = csv_data[csv_data.predict_sort == 0.0]
